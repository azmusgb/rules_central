...
class CatalogViewer {
  ...
  setupEventListeners() {
    // Search
    this.uiElements.searchInput?.addEventListener("input", () =>
      this.handleSearchInput(),
    );
    this.uiElements.clearSearchBtn?.addEventListener("click", () =>
      this.clearSearch(),
    );
    this.uiElements.searchButton?.addEventListener("click", () =>
      this.filterCatalog(),
    );
    this.uiElements.resetButton?.addEventListener("click", () =>
      this.resetFilters(),
    );
    this.uiElements.resetSearchButton?.addEventListener("click", () =>
      this.resetFilters(),
    );

    // Expand/collapse
    this.uiElements.expandAllButton?.addEventListener("click", () =>
      this.expandAll(),
    );
    this.uiElements.collapseAllButton?.addEventListener("click", () =>
      this.collapseAll(),
    );

    // Dropdown toggles
    this.uiElements.filterToggle?.addEventListener("click", (e) =>
      this.toggleDropdown(e, "filterMenu"),
    );
    this.uiElements.sortToggle?.addEventListener("click", (e) =>
      this.toggleDropdown(e, "sortMenu"),
    );

    // Help modal
    this.uiElements.helpButton?.addEventListener("click", () =>
      this.toggleQuickHelp(),
    );
    this.uiElements.closeHelpModal?.addEventListener("click", () =>
      this.toggleQuickHelp(),
    );
    ...
    document.addEventListener("click", (e) => {
      if (
        this.uiElements.filterMenu &&
        !this.uiElements.filterMenu.contains(e.target) &&
        e.target !== this.uiElements.filterToggle
      ) {
        this.uiElements.filterMenu.classList.add("hidden");
      }
      if (
        this.uiElements.sortMenu &&
        !this.uiElements.sortMenu.contains(e.target) &&
    ...
    // View mode toggle
    this.uiElements.viewToggle?.addEventListener("click", () =>
      this.toggleViewMode(),
    );
  ...
  setupIntersectionObserver() {
    ...
    document
      .querySelectorAll(".group-section, .subgroup-section")
      .forEach((el) => {
        observer.observe(el);
      });
  }
  ...
  toggleQuickHelp() {
    ...
    document.body.style.overflow =
      this.uiElements.quickHelp?.classList.contains("hidden") ? "" : "hidden";
  }
  ...
  showLoading(show) {
    if (!this.uiElements.loadingIndicator || !this.uiElements.catalogContainer)
      return;
  ...
  createCategoryNavigation() {
    if (!this.allCatalogs?.length || !this.uiElements.categoryNavContainer)
      return;
    ...
    Array.from(categories)
      .sort(
        (a, b) => (this.GROUP_PRIORITY[b] || 0) - (this.GROUP_PRIORITY[a] || 0),
      )
      .forEach((category) => {
  ...
  filterByCategory(category) {
    ...
    const buttons =
      this.uiElements.categoryNavContainer?.querySelectorAll("button");
    buttons?.forEach((btn) => {
      const isActive =
  ...
  renderCatalog(catalogs) {
    ...
    Object.entries(groupedCatalogs)
      .sort(
        ([a], [b]) =>
          (this.GROUP_PRIORITY[b] || 0) - (this.GROUP_PRIORITY[a] || 0),
      )
    ...
    // Update result count
    const totalItems = Object.values(groupedCatalogs).reduce(
      (sum, group) =>
        sum +
        Object.values(group.subgroups).reduce(
          (subSum, entries) => subSum + entries.length,
          0,
        ),
      0,
    );
  ...
  groupCatalogs(catalogs) {
    return catalogs.reduce((groups, cat) => {
      ...
      const normalizedTop =
        this.findSimilarGroup(Object.keys(groups), top) || top;
  ...
  createGroupSection(group, groupData) {
    ...
    groupHeader.className =
      "group-header flex items-center justify-between p-4 rounded-lg bg-dark-800 border border-dark-700 cursor-pointer transition-all hover:border-primary-500/30 hover:shadow-lg";
    ...
    groupHeader.addEventListener("click", () => {
      ...
      const folderIcon = groupHeader.querySelector(
  ...
  createSubgroupCard(groupName, subgroup, entries) {
    const subgroupId = `${groupName}-${subgroup}`
      .toLowerCase()
      .replace(/\s+/g, "-");
    const isSubgroupExpanded =
      this.filterState.expandedSubgroups.has(subgroupId);
    const subgroupSection = document.createElement("div");
    subgroupSection.className = "subgroup-section mb-4 animate-fadeIn";
    const subgroupHeader = document.createElement("div");
    subgroupHeader.className =
      "subgroup-header flex items-center justify-between p-3 rounded-lg bg-dark-800/50 border border-dark-700 cursor-pointer transition-all hover:border-primary-500/30 hover:shadow-md";
  ...
  createEntryItem(entry) {
    ...
    card.className =
      "entry-item group relative bg-gradient-to-br from-gray-800/80 to-gray-900/90 rounded-xl overflow-hidden border border-gray-700/30 hover:border-purple-500/30 transition-all duration-300 hover:shadow-glow-sm flex flex-col h-full transform hover:-translate-y-1 animate-fadeIn";
    card.title = entry.description || "No description available";
    card.dataset.searchTerms =
      `${entry.root} ${entry.diagram} ${entry.hierarchy}`.toLowerCase();
    ...
    card.innerHTML = `
            <div class="p-4 flex-grow flex flex-col relative">
                ...
                <div class="flex flex-wrap gap-2 mt-auto pt-2">
                    ${(entry.tags || ["#process", "#flow", "#business"])
                      .map(
                        (tag) =>
  ...
  handleSearchInput() {
    this.filterState.searchTerm = this.uiElements.searchInput.value
      .toLowerCase()
      .trim();
    if (this.uiElements.clearSearchBtn) {
      const shouldShowClear = this.filterState.searchTerm.length > 0;
      this.uiElements.clearSearchBtn.classList.toggle(
        "opacity-0",
        !shouldShowClear,
      );
      this.uiElements.clearSearchBtn.classList.toggle(
  ...
  filterCatalog() {
      ...
      subgroups.forEach((subgroup) => {
        ...
        entries.forEach((entry) => {
          ...
          const matchesSearch =
            ...
            subgroupName.includes(searchTerm) ||
    ...
    if (anyVisible) {
      const firstVisible = document.querySelector(
        '.group-section:not([style*="display: none"])',
      );
  ...
  resetFilters() {
    ...
    this.filterState.diagramTypes = new Set(
      Array.from(this.uiElements.filterCheckboxes).map((cb) => cb.value),
    );
