document.addEventListener('DOMContentLoaded', () => {
    ...
    function formatFilename(filename) {
        return filename ? filename.replace(/\.mmd$/, '').replace(/_/g, ' ') : 'Untitled';
    }

    function formatCatalogName(catalog) {
        return catalog ? catalog.replace(/^Documents_/, '').replace(/_/g, ' ') : 'Unknown Catalog';
    }

    function formatDate(dateValue) {
        if (!dateValue) return 'Unknown';
        const date = new Date(dateValue * 1000 || dateValue);
        return isNaN(date) ? 'Unknown' : date.toLocaleDateString('en-US', {
            year: 'numeric',
            month: 'short',
            day: 'numeric'
        });
    ...
    function createRuleDetails(item, query) {
        ...
        const ruleName = ruleData.RuleName || extractRuleNameFromSnippet(item.match_snippet) || formatFilename(item.filename);
        const ruleGuid = ruleData.RuleGUID || extractGuidFromSnippet(item.match_snippet);
        const functionName = ruleData.FunctionName || extractFunctionFromSnippet(item.match_snippet);
    ...
    async function performSearch() {
        ...
        console.debug('Performing search', { query, catalog, page: state.currentPage });
    ...
    function displayResults(items, query, catalog) {
        ...
        if (elements.resultsSummary && elements.resultsCount && elements.queryDisplay) {
        ...
        items.forEach((item) => {
            try {
                ...
                card.querySelector('.toggle-details-btn').addEventListener('click', () => {
    ...
    function resetResults() {
        ...
        if (elements.resultsSummary) elements.resultsSummary.classList.add('hidden');
        if (elements.pagination) elements.pagination.classList.add('hidden');
        ...
        if (elements.searchInput) {
            elements.searchInput.addEventListener('input', () => {
                if (elements.clearInput) {
                    elements.clearInput.classList.toggle('hidden', elements.searchInput.value === '');
                }
                debouncedSearch();
            });
        }
