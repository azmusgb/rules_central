...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function saveConfig() {
    try {
      ...
      elements.saveBtn.innerHTML =
        '<span class="material-icons mr-2 animate-spin">autorenew</span> Saving...';
  ...
  function updateUI() {
    ...
    if (config.currentTheme) {
      ...
      document.body.style.backgroundColor =
        theme.defaults?.background || "#ffffff";
  ...
  function renderThemeList() {
    ...
    const filteredThemes = Object.entries(config.themes).filter(([name]) =>
  ...
  function setupColorPickers() {
    document.querySelectorAll(".color-picker").forEach((pickerEl) => {
      ...
      if (config.currentTheme && config.themes[config.currentTheme]) {
        const classDefs = parseClassDefs(
          config.themes[config.currentTheme].classDefs,
        );
        if (classDefs[target]) {
  ...
  function deleteTheme(themeName) {
    if (
      ...
      Object.keys(config.themes).length <= 1
    ) {
  ...
  function setupEventListeners() {
    ...
    elements.newThemeBtn?.addEventListener("click", () => {
      ...
      elements.baseThemeSelect.innerHTML =
    ...
    Object.values(elements.nodePreviews).forEach((node) => {
      node?.addEventListener("click", () => {
        ...
        const advancedTabBtn = tabBtns.find(
          (btn) => btn.dataset.tab === "advanced",
        );
        if (advancedTabBtn) {
          tabBtns.forEach((btn) =>
    ...
    document
      .querySelectorAll('[data-action="edit-text-color"]')
      .forEach((btn) => {
    ...
    document
      .querySelectorAll('[data-action="edit-border-color"]')
      .forEach((btn) => {
        btn?.addEventListener("click", (e) => {
          if (isNavigationLink(e.target)) return;
          const target = e.target.closest("[data-target]").dataset.target;
    ...
    document.addEventListener("click", (e) => {
      if (
        !elements.newThemeModal.classList.contains("hidden") &&
