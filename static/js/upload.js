...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const showToast = (msg, type = "info") => {
    window?.app?.showToast?.(msg, type) ?? console.log(`[${type}] ${msg}`);
  };
  ...
  const calculateTotalSize = (fileList) =>
    Array.from(fileList).reduce((total, f) => total + f.size, 0);

  const filterValidFiles = (files) =>
    Array.from(files).filter((file) => file.name.toLowerCase().endsWith(ALLOWED_EXT));

  const updateFileListInput = (files) => {
  ...
  function updateFilePreview() {
    ...
    totalSizeDisplay.textContent = formatFileSize(totalBytes);
    ...
    files.forEach((file, index) => {
      ...
      card.className = "file-card bg-gray-50 p-3 rounded-lg flex items-center justify-between";

      const infoWrap = document.createElement("div");
      infoWrap.className = "flex items-center";
      ...
      name.className = "text-sm font-medium text-gray-700 truncate max-w-xs";
      name.textContent = file.name;

      const size = document.createElement("span");
      size.className = "text-xs text-gray-500 ml-2";
      ...
      infoWrap.append(icon, name, size);
      card.append(infoWrap, removeBtn);
  ...
  function removeFile(index) {
    ...
    updateFileListInput(files);
    updateFilePreview();
  }

  // ===== Drag & Drop Handling =====

  const activateDropZone = () => dropZone.classList.add("active");
  const deactivateDropZone = () => dropZone.classList.remove("active");
  ...
  dropZone.addEventListener("focus", activateDropZone);
  dropZone.addEventListener("blur", deactivateDropZone);
  ...
  dropZone.addEventListener("drop", (e) => {
    ...
    if (validFiles.length) {
      updateFileListInput(validFiles);
      updateFilePreview();
    }
  });
  ...
  form.addEventListener("submit", (e) => {
    ...
    if (!input.files.length) {
      showToast("Please select at least one JSON file", "error");
      return;
    }
    ...
    xhr.upload.addEventListener("progress", (e) => {
      ...
      progressStatus.textContent = percent < 100 ? "Uploading..." : "Processing files...";
    });
    ...
    xhr.addEventListener("load", () => {
      ...
      if (response.success) {
        ...
        showToast("JSON files uploaded successfully!", "success");
        setTimeout(() => {
          const redirectUrl =
            response.redirect_url ||
            `/view_diagram?root_name=${encodeURIComponent(
              input.files[0].name.replace(/\.[^/.]+$/, "")
            )}&diagramName=${encodeURIComponent(
              input.files[0].name.replace(/\.[^/.]+$/, "")
            )}.mmd`;
