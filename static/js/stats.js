...
class StatsManager {
  constructor() {
    ...
    if (this.statsElements.topContributor) {
      this.statsElements.contributorName =
        this.statsElements.topContributor.querySelector(".contributor-name");
      this.statsElements.contributorActions =
        this.statsElements.topContributor.querySelector(".contributor-actions");
    }
  ...
  async updateAllStats() {
    try {
      await Promise.all([this.updateBasicStats(), this.updateAdvancedStats()]);
      this.lastUpdate = new Date();
      console.log(
        `Stats successfully updated at ${this.lastUpdate.toLocaleTimeString()}`,
      );
    } catch (error) {
  ...
  async updateBasicStats() {
    try {
      ...
      if (
        this.statsElements.diagramsProcessed &&
        data.diagramsProcessed !== undefined
      ) {
        this.animateCount(
          this.statsElements.diagramsProcessed,
          data.diagramsProcessed,
        );
      }

      // Update rules count with animation
      if (
        this.statsElements.rulesExtracted &&
        data.rulesExtracted !== undefined
      ) {
        this.animateCount(
          this.statsElements.rulesExtracted,
          data.rulesExtracted,
  ...
  async updateAdvancedStats() {
    try {
      const advancedKeys = Object.keys(this.statsElements).filter(
        (k) =>
          !["diagramsProcessed", "rulesExtracted", "topContributor"].includes(
            k,
          ),
      );
  ...
  updateAdvancedStatsElements(data) {
    ...
    if (data.status_counts) {
      if (this.statsElements.activeRulesCount) {
        this.animateCount(
          this.statsElements.activeRulesCount,
          data.status_counts.active,
        );
      }
      if (this.statsElements.draftRulesCount) {
        this.animateCount(
          this.statsElements.draftRulesCount,
          data.status_counts.draft,
        );
      }
      if (this.statsElements.deprecatedRulesCount) {
        this.animateCount(
          this.statsElements.deprecatedRulesCount,
          data.status_counts.deprecated,
        );
      }
    }
    ...
    if (data.recent_activity) {
      if (this.statsElements.recentChangesCount) {
        this.animateCount(
          this.statsElements.recentChangesCount,
          data.recent_activity.total,
        );
      }
      if (this.statsElements.creationsCount) {
        this.animateCount(
          this.statsElements.creationsCount,
          data.recent_activity.creations,
        );
      }
      if (this.statsElements.updatesCount) {
        this.animateCount(
          this.statsElements.updatesCount,
          data.recent_activity.updates,
        );
      }
      if (this.statsElements.deletionsCount) {
        this.animateCount(
          this.statsElements.deletionsCount,
          data.recent_activity.deletions,
        );
      }
    }
    ...
    if (data.top_contributor) {
      if (this.statsElements.contributorName) {
        this.statsElements.contributorName.textContent =
          data.top_contributor.user || "None";
      }
      if (this.statsElements.contributorActions) {
        this.statsElements.contributorActions.textContent = `${data.top_contributor.actions || 0} actions`;
      }
    }
  ...
  showLoading(keys) {
    keys.forEach((key) => {
      if (this.statsElements[key]) {
        this.statsElements[key].innerHTML =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (
    document.getElementById("diagramsProcessed") ||
    document.getElementById("activeRulesCount")
  ) {
