...
document.addEventListener("DOMContentLoaded", function () {
  ...
  async function loadActivityStats() {
    ...
    try {
      ...
      if (entries.length === 0) {
        statsList.innerHTML =
          '<li class="text-gray-400">No activity data available.</li>';
      } else {
        entries.forEach(([catalog, diagrams], idx) => {
          const li = document.createElement("li");
          li.className =
            "flex items-center justify-between bg-dark-800/80 rounded-lg px-6 py-4 shadow transition hover:bg-dark-700/90";
          li.innerHTML = `
          ...
          setTimeout(
            () => {
              ...
              const percent = max
                ? Math.round((diagrams.length / max) * 100)
                : 0;
