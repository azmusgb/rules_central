...
def get_snippet(text: str, term: str, snippet_length: int = DEFAULT_SNIPPET_LENGTH) -> str:
...
def generate_files(rules: RuleListType, out_dir: str) -> List[DiagramInfo]:
    ...
    for rule in rules:
        ...
        infos.append(DiagramInfo(filename=mmd.name, created=datetime.now(timezone.utc).isoformat(), size=mmd.stat().st_size))
    return infos
...
def generate_mermaid_code(nodes: Dict[str, Dict[str, str]], edges: List[Dict[str, str]]) -> str:
...
def log_activity(action: str, rule_id: Optional[str] = None, user: str = "", details: str = "") -> None:
    ...
    log.append({"action": action, "rule_id": rule_id, "user": user, "details": details, "timestamp": datetime.now(timezone.utc).isoformat()})
    if rule_id:
...
def get_rule_stats() -> Dict[str, int]:
    ...
    def within(days: int) -> int:
        ...
        return sum(1 for a in activity if datetime.fromisoformat(a["timestamp"]) >= cutoff)
    return {
...
def get_activity_trend(days: int = 30) -> List[Dict[str, int]]:
    ...
    for i in range(days):
        ...
        count = sum(1 for a in activity if datetime.fromisoformat(a["timestamp"]).date() == day)
...
def flatten_rules(rules: RuleListType) -> RuleListType:
    ...
    def _walk(rule: RuleType, parent: Optional[str], action_idx: Optional[int]):
        r = {"RuleGUID": rule.get("RuleGUID"), "ParentGUID": parent, "ParentActionIndex": action_idx}
        flat.append(r)
        for child in rule.get("Children", []):
...
def validate_hierarchy_data(data: List[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, Dict[str, Any]]]:
...
def build_hierarchy(normalized: List[Dict[str, Any]], guid_map: Dict[str, Dict[str, Any]]) -> List[Dict[str, Any]]:
...
__all__ = [
    ...
    "get_diagram_categories", "get_dynamic_groups", "generate_mermaid_code",
